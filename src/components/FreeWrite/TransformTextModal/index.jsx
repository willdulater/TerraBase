...
const TransformTextModal = ({
	...
	const [textToTransform, setTextToTransform] = useState(transformText)
	const [transformationCommand, setTransformationCommand] = useState("")

	useEffect(() => {
		setTextToTransform(transformText)
	}, [transformText])
	...
	useEffect(() => {
		if (!open) {
			setTimeout(() => {
				setTextToTransform(transformText)
				setTransformationCommand("")
			}, 500)
		}
	}, [open])

	const submitTransformText = () => {
		if (textToTransform.length < 5) {
			toast.error(
				"Please select a word or phrase from your writing to transform!"
			)
			return
		}
		...
		socket.send(
			JSON.stringify({
				...
				transformation_command: transformationCommand,
								...
								<div className='flex flex-col mt-3 space-y-3 min-h-[4rem]'>
									<div className='flex flex-col'>
										...
										<TextareaAutosize
											...
											onChange={(e) => setTextToTransform(e.target.value)}
											placeholder='Select text from your writing to transform'
										/>
									</div>
								</div>
								...
								<div className='flex flex-col mt-3 space-y-3'>
									<div className='flex flex-col'>
										...
										<TextareaAutosize
											...
											value={transformationCommand}
											onChange={(e) => setTransformationCommand(e.target.value)}
											placeholder='Include references to museums across New York City'
										/>
									</div>
								</div>
...
export default TransformTextModal
